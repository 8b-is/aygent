...
feedback_processed = Counter(
    "feedback_items_processed_total", "Total feedback items processed"
)
feedback_errors = Counter("feedback_errors_total", "Total errors processing feedback")
github_issues_created = Counter(
    "github_issues_created_total", "Total GitHub issues created"
)
duplicate_detected = Counter(
    "feedback_duplicates_detected_total", "Duplicate feedback detected"
)
processing_time = Histogram(
    "feedback_processing_seconds", "Time spent processing feedback"
)
        ...
        try:
            ...
            for issue in issues:
                ...
                issue_fingerprint = self.generate_fingerprint(
                    {"title": issue.title, "description": issue.body or ""}
                )
    ...
    async def process_feedback(self, feedback: Dict):
        ...
        with processing_time.time():
            try:
                ...
                logger.info(
                    f"Processing {category}: {feedback.get('title', 'Untitled')}"
                )
                ...
                if issue_number:
                    ...
                    async with self.session.post(
                        ...
                        if resp.status != 200:
                            logger.error(
                                f"Failed to mark feedback as processed: {resp.status}"
                            )
    ...
    async def fetch_pending_feedback(self) -> List[Dict]:
        ...
        try:
            async with self.session.get(
                f"{self.feedback_api_url}/feedback/pending"
            ) as resp:
                if resp.status == 200:
